...
from src.model import CustomLossTrainer, compute_metrics
...
def main():
    ...
    print("1. Đang tải Dữ liệu và tính toán Trọng số điều hòa...")
    data_module = EmotionDataModule(config['data']['processed_dir'], num_labels)
    train_ds, val_ds, _ = data_module.get_datasets()
    
    # Đọc tham số alpha làm dịu từ cấu hình YAML
    smoothing_alpha = config['training'].get('smoothing_alpha', 1.0)
    class_weights_tensor = data_module.compute_class_weights(smoothing_alpha=smoothing_alpha)

    print(" -> Trọng số điều hòa lớp (Calibrated Weights) thành công:")
    for i, label in id2label.items():
        print(f"    - {label:<10} : {class_weights_tensor[i]:.4f}")

    print("\n2. Khởi tạo mô hình RoBERTa-base...")
    ...
    print(f"\n✅ Hoàn tất! Mô hình tối ưu nâng cao đã được lưu tại: {config['training']['final_model_dir']}")
